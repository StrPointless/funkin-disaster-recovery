...
class NoteOffsetState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (FlxG.save.data.finishedWeekScrewie == true){
		screwie = new Character(220, 50, 'screwiehd');
		screwie.x += screwie.positionArray[0];
		screwie.y += screwie.positionArray[1];
		screwie.scrollFactor.set(0.95, 0.95);
		add(screwie);
		}
	...
	override public function update(elapsed:Float)
	{
		...
		if(controls.BACK)
		{
			...
			if (ClientPrefs.transitionStyle && !ClientPrefs.legacyEngine){
				...
				if (FlxG.save.data.inNoteOffest == true){
				MusicBeatState.switchState(new PlayState());
				} else {
				MusicBeatState.switchState(new options.OptionsState());
				FlxG.sound.playMusic(Paths.music('/mainMenu/' + ClientPrefs.mainMenuMusic), 1, true);
				}
				});
				} else {
				if (FlxG.save.data.inNoteOffest == true){
