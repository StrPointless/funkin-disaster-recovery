...
class ChartingState extends MusicBeatState
{
	...
	private var strumLine2:FlxSprite;
	...
	var gridBGGradient:FlxSprite;
	var nextGridBGGradient:FlxSprite;

	var bg:FlxSprite;

	//I know there are better ways to do this, but whatever, it works fine - Luci
	...
	**/
	...
	override function create()
	{
		...
		Application.current.window.title = "Friday Night Funkin': A Funkin' Disaster - In Chart Editor - " + StringTools.replace(_song.song, '-', ' ');
		#end

		boyfriend = new Boyfriend(0, 0, _song.player1);
		startCharacterLua(boyfriend.curCharacter);
		gf = new Character(0, 0, _song.gfVersion);
		...
		strumLine2 = new FlxSprite(0, 0).makeGraphic(Std.int(GRID_SIZE * 9), 2, FlxColor.BLACK);
	...
	var playersGridColor:FlxUICheckBox = null; //The actual check box - Luci
	var UI_songTitle:FlxUIInputText;
	var noteSkinInputText:FlxUIInputText;
	var noteSplashesInputText:FlxUIInputText;
	var stageDropDown:FlxUIDropDownMenuCustom;

	public static var gridColorCheck:Bool = false;//The bool to use with the check box to make it work - Luci
	...
	function addChartingUI() {
		...
		var playersGridColor = new FlxUICheckBox(check_mute_inst.x + 120, 125, null, null, "Change grid color to the characters color?", 100);
		if (FlxG.save.data.playersGridColor == null) FlxG.save.data.playersGridColor = false;
		playersGridColor.checked = FlxG.save.data.playersGridColor;

		gridColorCheck = FlxG.save.data.playersGridColor;

		playersGridColor.callback = function()
			{
				FlxG.save.data.playersGridColor = playersGridColor.checked;
				gridColorCheck = FlxG.save.data.playersGridColor;
		};	
		...
		tab_group_chart.add(playersGridColor);
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			if (FlxG.keys.justPressed.Q)
				{
					if (FlxG.keys.pressed.SHIFT)
						curSec = Std.int(FlxG.sound.music.length / 1000);
				}
		...
		/*I had to put this here BECAUSE whenever the characters are changed in chart editor
		The background color won't update, there's probably a better way to do this, but eh, it works. - Luci*/
		boyfriend = new Boyfriend(0, 0, _song.player1);
		startCharacterLua(boyfriend.curCharacter);
		gf = new Character(0, 0, _song.gfVersion);
		startCharacterLua(gf.curCharacter);
		...
		if(gridColorCheck){
			...
			bg.color = FlxColor.fromRGB(gf.healthColorArray[0] - 50, gf.healthColorArray[1] - 50, gf.healthColorArray[2] - 50);
			strumLine.color = FlxColor.fromRGB(FlxColor.fromRGB(gf.healthColorArray[0], gf.healthColorArray[1], gf.healthColorArray[2]), boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]);
		} else if (check_mustHitSection.checked){
			bg.color = FlxColor.fromRGB(boyfriend.healthColorArray[0] - 50, boyfriend.healthColorArray[1] - 50, boyfriend.healthColorArray[2] - 50);
			strumLine.color = FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]);
		} else {
			bg.color = FlxColor.fromRGB(dad.healthColorArray[0] - 50, dad.healthColorArray[1] - 50, dad.healthColorArray[2] - 50);
			strumLine.color = FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]);
		}
		} else {
			remove(strumLine2);
			bg.color = 0xFF222222;
	...
	*/
	...
	function reloadGridLayer() {
		...
		if (gridColorCheck){
			...
			gridBGGradient = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * 9, Std.int(GRID_SIZE * getSectionBeats() * 4 * zoomList[curZoom]));

			if(check_gfSection.checked && !check_mustHitSection.checked)
			gridBGGradient = FlxGradient.createGradientFlxSprite(Std.int(gridBG.width), Std.int(gridBG.height), [FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]), FlxColor.fromRGB(gf.healthColorArray[0], gf.healthColorArray[1], gf.healthColorArray[2])], 1, 270, true);
			else if(check_gfSection.checked && check_mustHitSection.checked)
			gridBGGradient = FlxGradient.createGradientFlxSprite(Std.int(gridBG.width), Std.int(gridBG.height), [FlxColor.fromRGB(gf.healthColorArray[0], gf.healthColorArray[1], gf.healthColorArray[2]), FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2])], 1, 270, true);
			else if(check_mustHitSection.checked)
			gridBGGradient = FlxGradient.createGradientFlxSprite(Std.int(gridBG.width), Std.int(gridBG.height), [FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]), FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2])], 1, 270, true);
			else if(!check_mustHitSection.checked && check_gfSection.checked)
			gridBGGradient = FlxGradient.createGradientFlxSprite(Std.int(gridBG.width), Std.int(gridBG.height), [FlxColor.fromRGB(gf.healthColorArray[0], gf.healthColorArray[1], gf.healthColorArray[2]), FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2])], 1, 270, true);
			else if(!check_mustHitSection.checked && !check_gfSection.checked)
			gridBGGradient = FlxGradient.createGradientFlxSprite(Std.int(gridBG.width), Std.int(gridBG.height), [FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]), FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2])], 1, 270, true);
			
			gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * 9, Std.int(GRID_SIZE * getSectionBeats() * 4 * zoomList[curZoom]), true, 0xFF000000, 0xffffffff);
			gridBG.alpha = 0.5;
		} else {
	...
	if (gridColorCheck){
		...
		nextGridBGGradient = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * 9, Std.int(GRID_SIZE * getSectionBeats() * 4 * zoomList[curZoom]));

			if(check_gfSection.checked && !check_mustHitSection.checked)
			nextGridBGGradient = FlxGradient.createGradientFlxSprite(Std.int(gridBG.width), Std.int(gridBG.height), [FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]), FlxColor.fromRGB(gf.healthColorArray[0], gf.healthColorArray[1], gf.healthColorArray[2])], 1, 270, true);
			else if(check_gfSection.checked && check_mustHitSection.checked)
			nextGridBGGradient = FlxGradient.createGradientFlxSprite(Std.int(gridBG.width), Std.int(gridBG.height), [FlxColor.fromRGB(gf.healthColorArray[0], gf.healthColorArray[1], gf.healthColorArray[2]), FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2])], 1, 270, true);
			else if(check_mustHitSection.checked)
			nextGridBGGradient = FlxGradient.createGradientFlxSprite(Std.int(gridBG.width), Std.int(gridBG.height), [FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]), FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2])], 1, 270, true);
			else if(!check_mustHitSection.checked && check_gfSection.checked)
			nextGridBGGradient = FlxGradient.createGradientFlxSprite(Std.int(gridBG.width), Std.int(gridBG.height), [FlxColor.fromRGB(gf.healthColorArray[0], gf.healthColorArray[1], gf.healthColorArray[2]), FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2])], 1, 270, true);
			else if(!check_mustHitSection.checked && !check_gfSection.checked)
			nextGridBGGradient = FlxGradient.createGradientFlxSprite(Std.int(gridBG.width), Std.int(gridBG.height), [FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]), FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2])], 1, 270, true);

			nextGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * 9, Std.int(GRID_SIZE * getSectionBeats(curSec + 1) * 4 * zoomList[curZoom]), true, 0xFF000000, 0xffffffff);
			nextGridBG.alpha = 0.5;
		...
		var gridGradientLine:FlxSprite = new FlxSprite(gridBG.x + GRID_SIZE).makeGraphic(2, leHeight, FlxColor.WHITE);
		var gridGradientLine2:FlxSprite = new FlxSprite(gridBG.x).makeGraphic(2, leHeight, FlxColor.WHITE);
		var gridBlackLine2:FlxSprite = new FlxSprite(gridBG.x + 5).makeGraphic(2, leHeight, FlxColor.BLACK);

		if(gridColorCheck){
			if(check_gfSection.checked && check_mustHitSection.checked){
				gridGradientLine = FlxGradient.createGradientFlxSprite(Math.round(2), (leHeight), [FlxColor.fromRGB(gf.healthColorArray[0], gf.healthColorArray[1], gf.healthColorArray[2]), FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2])], 1, 270, true);
				gridLayer.add(gridGradientLine);
				gridGradientLine2 = FlxGradient.createGradientFlxSprite(Math.round(2), (leHeight), [FlxColor.fromRGB(gf.healthColorArray[0], gf.healthColorArray[1], gf.healthColorArray[2]), FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2])], 1, 270, true);
				gridLayer.add(gridGradientLine2);
				//gridBlackLine2 = FlxGradient.createGradientFlxSprite(Math.round(2), (leHeight), [FlxColor.fromRGB(gf.healthColorArray[0], gf.healthColorArray[1], gf.healthColorArray[2]), FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2])], 1, 270, true);
				//gridLayer.add(gridBlackLine2);
			} else if(check_gfSection.checked && !check_mustHitSection.checked){
				gridGradientLine = FlxGradient.createGradientFlxSprite(Math.round(2), (leHeight), [FlxColor.fromRGB(gf.healthColorArray[0], gf.healthColorArray[1], gf.healthColorArray[2]), FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2])], 1, 270, true);
				gridLayer.add(gridGradientLine);
				gridGradientLine2 = FlxGradient.createGradientFlxSprite(Math.round(2), (leHeight), [FlxColor.fromRGB(gf.healthColorArray[0], gf.healthColorArray[1], gf.healthColorArray[2]), FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2])], 1, 270, true);
				gridLayer.add(gridGradientLine2);
				//gridBlackLine2 = FlxGradient.createGradientFlxSprite(Math.round(2), (leHeight), [FlxColor.fromRGB(gf.healthColorArray[0], gf.healthColorArray[1], gf.healthColorArray[2]), FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2])], 1, 270, true);
				//gridLayer.add(gridBlackLine2);
			} else if (check_mustHitSection.checked){
				gridGradientLine = FlxGradient.createGradientFlxSprite(Math.round(2), (leHeight), [FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]), FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2])], 1, 270, true);
				gridLayer.add(gridGradientLine);
				gridGradientLine2 = FlxGradient.createGradientFlxSprite(Math.round(2), (leHeight), [FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]), FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2])], 1, 270, true);
				gridLayer.add(gridGradientLine2);
				//gridBlackLine2 = FlxGradient.createGradientFlxSprite(Math.round(2), (leHeight), [FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]), FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2])], 1, 270, true);
				//gridLayer.add(gridBlackLine2);
			} else if (!check_mustHitSection.checked && !check_gfSection.checked){
				gridGradientLine = FlxGradient.createGradientFlxSprite(Math.round(2), (leHeight), [FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]), FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2])], 1, 270, true);
				gridLayer.add(gridGradientLine);
			...
			var gridBlackLine2:FlxSprite = new FlxSprite(gridBG.x + GRID_SIZE).makeGraphic(2, leHeight, FlxColor.BLACK);
			gridLayer.add(gridBlackLine2);
			gridBlackLine2.x = gridBG.x + 100;
		}
	}
		gridGradientLine.x = gridBG.x + GRID_SIZE;
		gridGradientLine2.x = gridBG.x;
		gridBlackLine2.x = gridBG.x + 360;
