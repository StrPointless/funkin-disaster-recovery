...
class DialogueCharacterEditorState extends MusicBeatState
{
	...
	function reloadCharacter() {
		...
		Application.current.window.title = "Friday Night Funkin': A Funkin' Disaster - In Dialogue Character Editor - " + character.jsonFile.image;
		#end
	}
	...
	override function update(elapsed:Float) {
		...
		if(!blockInput && !animationDropDown.dropPanel.visible) {
			...
			if(FlxG.keys.justPressed.ESCAPE) {
				...
				FlxG.sound.playMusic(Paths.music('/mainMenu/' + ClientPrefs.mainMenuMusic), 1);
				transitioning = true;
			}
