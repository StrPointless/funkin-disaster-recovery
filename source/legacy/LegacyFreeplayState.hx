...
{
	var songs:Array<SongMetadataClone> = [];
	...
	override function create()
	{
		...
		Application.current.window.title = "Friday Night Funkin': A Funkin' Disaster - In Freeplay (Legacy Edition)";
		#end
		...
		if (TitleState.unlockedSongs[2])
			addSong('spectrum', 0, 'spectrum', 0xFFA8A8A8);
		...
		scoreBG = new FlxSprite(scoreText.x - 6, 628).makeGraphic(1, 66, 0xFF000000);
		scoreBG.alpha = 0.6;
		add(scoreBG);
	...
	{
		songs.push(new SongMetadataClone(songName, weekNum, songCharacter, color));
	}
